...
// Tipo explícito (reto): describe nombre + stock; evita object[].
type Producto = {
  nombre: string;
  stock: number;
};
...
const agregarProducto = (productos: Producto[], nuevoProducto: Producto) => {
  return [...productos, nuevoProducto];
};

const catalogo: Producto[] = [{ nombre: "Teclado", stock: 3 }];
