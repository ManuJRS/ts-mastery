...
function saludar(nombre: string, momento: string = "buenos días"): string {
...
function totalCalculate(price:number, tax:number | 0.21): number {
    return price*tax
} 
...
function calcularTotal(precio: number, impuesto: number = 0.21): number {
...
const convertirTexto = (texto: string, modo: "mayuscula" | "minuscula" ) => {
    if (modo === "mayuscula") {
...
const ultimoElemento = (array: string[]):string | undefined => {
